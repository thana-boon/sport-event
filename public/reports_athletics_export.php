...
// ดึงข้อมูลการแข่งขัน (ครั้งที่, วันที่)
$metaSt = $pdo->prepare("SELECT edition_no, start_date, end_date, title FROM competition_meta WHERE year_id=:y LIMIT 1");
$metaSt->execute([':y'=>$yearId]);
$meta = $metaSt->fetch(PDO::FETCH_ASSOC);

$editionNo = $meta['edition_no'] ?? '';
$startDate = $meta['start_date'] ?? null;
$endDate = $meta['end_date'] ?? null;
$competitionTitle = $meta['title'] ?? '';

// ใช้ครั้งที่จาก competition_meta (ตั้งค่าที่หน้า reports_booklet.php)
$gameNumber = $editionNo !== '' ? $editionNo : '-';
...
if ($startDate && $endDate) {
  ...
  $startYear = (int)$start->format('Y') + 543;
  
  $endDay = (int)$end->format('d');
  $endMonth = (int)$end->format('m');
  $endYear = (int)$end->format('Y') + 543;
  
  // รูปแบบ: "วันที่ 27 มกราคม 2569 ถึง 30 มกราคม 2569"
  $dateRangeText = 'วันที่ ' . $startDay . ' ' . $thaiMonths[$startMonth] . ' ' . $startYear . 
                   ' ถึง ' . $endDay . ' ' . $thaiMonths[$endMonth] . ' ' . $endYear;
...
<style>
  ...
  .logo{ height:54px; margin:0 auto 3px; display:inline-block; }
  h1{ font-size:20pt; margin:0 0 2px 0; text-align:center; line-height:1.1; }
  h2{ font-size:12.5pt; margin:1px 0 2px 0; text-align:center; color:#555; line-height:1.1; }
  ...
  <div class="header">
    ...
    <?php if($dateRangeText): ?>
      <div style="font-size:14pt; margin-top:1mm; line-height:1.1;"><?= htmlspecialchars($dateRangeText) ?></div>
    <?php endif; ?>
  </div>
