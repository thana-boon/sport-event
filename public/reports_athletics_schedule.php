...
// ✅ เพิ่ม team_size, participant_type และ competition_time ใน SELECT
$sql = "
  SELECT
    ae.event_code,
    ae.competition_time,
...
// ========================================
// ✅ สร้างตารางแข่งขัน (ใช้เวลาจาก DB หรือแสดง -)
// ========================================

$schedule = [];

foreach ($events as $event) {
    // ✅ ใช้เวลาจาก DB ถ้ามี ไม่เช่นนั้นแสดง "-"
    $displayTime = '-';
    if (!empty($event['competition_time'])) {
        $displayTime = substr($event['competition_time'], 0, 5); // HH:MM
    }
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td><?= $item['time'] === '-' ? '-' : htmlspecialchars($item['time'], ENT_QUOTES, 'UTF-8') . ' น.' ?></td>
        ...
        <table class="table table-bordered table-hover align-middle">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td class="text-center"><?= $item['time'] === '-' ? '-' : htmlspecialchars($item['time'], ENT_QUOTES, 'UTF-8') . ' น.' ?></td>
