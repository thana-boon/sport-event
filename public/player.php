...
if (isset($_SESSION['last_activity']) && (time() - $_SESSION['last_activity']) > $timeout) {
  log_activity('LOGOUT', 'users', $_SESSION['admin']['id'] ?? null, 
    'ออกจากระบบอัตโนมัติ (session timeout 60 นาที) | Username: ' . ($_SESSION['admin']['username'] ?? 'unknown'));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_GET['action'])) {
  ...
  try {
    ...
    if ($action === 'load_students') {
      ...
      // นับจำนวนที่ลงทะเบียนไปแล้ว (เฉพาะหมวดกีฬาเดียวกัน)
      $categoryId = $sport['category_id'];
      $countStmt = $pdo->prepare("
        SELECT r.student_id, COUNT(*) as count
        FROM registrations r
        JOIN sports sp ON sp.id = r.sport_id AND sp.year_id = r.year_id
        WHERE r.year_id = ? AND sp.category_id = ?
        GROUP BY r.student_id
      ");
      $countStmt->execute([$yearId, $categoryId]);
      $counts = [];
    ...
    if ($action === 'load_dashboard') {
      ...
      // ประวัติการเปลี่ยนตัวทั้งหมด (เรียงตามวันที่ สี ระดับชั้น และให้คนเดียวกันอยู่ติดกัน)
      $stmt = $pdo->prepare("
        SELECT 
          ps.*,
          sp.name as sport_name,
          ...
          old_s.first_name as old_fname,
          old_s.last_name as old_lname,
          old_s.class_level as old_class_level,
          new_s.student_code as new_code,
          new_s.first_name as new_fname,
          new_s.last_name as new_lname,
          new_s.class_level as new_class_level,
          u.username as created_by_name
        FROM player_substitutions ps
        JOIN sports sp ON ps.sport_id = sp.id
        JOIN students old_s ON ps.old_student_id = old_s.id
        ...
        WHERE ps.year_id = ?
        ORDER BY 
          DATE(ps.substitution_date) DESC,
          FIELD(ps.color, 'ส้ม', 'เขียว', 'ชมพู', 'ฟ้า'),
          new_s.class_level,
          new_s.first_name,
          new_s.last_name,
          sp.name
