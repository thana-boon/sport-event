...
<div class="container mt-3">
  <ul class="nav nav-tabs">
    <li class="nav-item">
      <a class="nav-link active" href="<?= BASE_URL ?>/referee/index.php">📝 บันทึกผล</a>
    </li>
    <li class="nav-item">
      <a class="nav-link" href="<?= BASE_URL ?>/referee/players.php">👥 รายชื่อนักกีฬา</a>
    </li>
  </ul>
</div>

<main class="container pb-4" style="margin-top: 0;")
  <!-- Progress Summary -->
  <div class="card shadow-sm mb-3" style="background: linear-gradient(135deg, #fff 0%, #f8fafc 100%); border-left: 4px solid var(--brand);">
    <div class="card-body">
      <div class="row align-items-center">
        <div class="col-12 col-md-8">
          <h6 class="mb-2" style="color: var(--brand-700); font-weight: 600;">📊 ความคืบหน้าการบันทึกผล</h6>
          <div class="progress" style="height: 30px; border-radius: 15px; background: #e2e8f0;">
            <div id="progressBar" class="progress-bar progress-bar-striped progress-bar-animated" 
                 role="progressbar" 
                 style="background: linear-gradient(135deg, var(--brand) 0%, var(--brand-600) 100%); font-weight: 600; font-size: 1rem;"
        ...
        <div class="col-12 col-md-4 text-md-end mt-3 mt-md-0">
          <div class="d-flex flex-column gap-1">
            <span style="font-size: 0.9rem; color: #64748b;">
              <strong style="color: var(--success);" id="savedCount">0</strong> / <span id="totalCount">0</span> รายการ
            </span>
            <span style="font-size: 0.85rem; color: #94a3b8;">
              ⏳ เหลืออีก <strong id="remainingCount" style="color: var(--warning);">0</strong> รายการ
            </span>
          </div>
        </div>
      </div>
...
async function loadInit(){
  const baseUrl = '<?= BASE_URL ?>/referee/';
  const [sRes, cRes] = await Promise.all([ 
    fetch(baseUrl + 'api_sports.php'), 
    fetch(baseUrl + 'api_colors.php') 
  ]);
  const s = await sRes.json(); const c = await cRes.json();
  
  console.log('🔍 API Response:', s);
  console.log('📊 Sports count:', s.sports ? s.sports.length : 0);
  
  if(!s.ok) return toast(s.error||'โหลดกีฬาไม่สำเร็จ'); 
  SPORTS = s.sports||[];
  ...
  // โหลดค่าตัวกรองที่เก็บไว้ หรือใช้ default "ทั้งหมด"
  const savedCat = localStorage.getItem('referee_filter_cat');
  if (savedCat !== null) {
    selCat.value = savedCat;
  }
  // ถ้ายังไม่เคยเลือก → ใช้ค่าเริ่มต้น "ทั้งหมด" (value="")
...
// ฟังก์ชันอัปเดตสถิติความคืบหน้า
function updateProgressStats() {
  const total = SPORTS.length;
  const saved = SPORTS.filter(s => s.saved).length;
  ...
  if (progressBar) {
    ...
    if (percentage === 100) {
      ...
      progressBar.style.background = 'linear-gradient(135deg, var(--brand) 0%, var(--brand-600) 100%)';
...
async function openResult(sportId, isAthletics, sportName){
  ...
  try{
    const baseUrl = '<?= BASE_URL ?>/referee/';
    const r=await fetch(baseUrl + 'save.php?ajax=load_non_athletics&sport_id='+sportId,{cache:'no-store'});
  ...
  document.getElementById('btnSave').onclick = async ()=>{
    ...
    const baseUrl = '<?= BASE_URL ?>/referee/';
    const res=await fetch(baseUrl + 'save.php',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({type:'non_athletics',sport_id:sportId,ranks})});
...
async function openAthletics(sportId, sportName){
  ...
  const baseUrl = '<?= BASE_URL ?>/referee/';
  const r = await fetch(baseUrl + 'fetch_athletics.php?sport_id=' + sportId, {cache:'no-store'});
  const d = await r.json();
  ...
  const rows = (d.lanes||[]).map((l, idx) => {
    ...
    if (isRelay) {
      ...
      <tr style="background-color: ${bgColor} !important;" data-row-idx="${idx}">
        ...
        <td style="width:120px"><input type="text" class="form-control form-control-sm time-input" name="time[]" value="${l.time_sec ?? ''}"></td>
        <td style="width:100px"><input type="number" class="form-control form-control-sm rank-input" name="rank[]" value="${l.rank ?? ''}" min="1" data-lane="${l.lane_no}" data-row-idx="${idx}"></td>
        <td class="text-center" style="width:70px"><input type="checkbox" class="form-check-input record-check" name="is_record[]" ${l.is_record ? 'checked' : ''} data-name="${l.assigned_name || l.display_name || ''}" data-lane="${l.lane_no}"></td>
      </tr>`;
    }
  }).join('');

  const best = d.best || { holder:'', time_sec:'', year:'' };
  const yearBe = d.year_be || (new Date().getFullYear() + 543); // ใช้ปีการศึกษาจาก API หรือปีปัจจุบัน
    ...
    <div class="border-top pt-3 mt-3">
      <div class="row g-3">
        ...
        <div class="col-6 col-md-4">
          ...
          <input id="bestYear" class="form-control form-control-sm" value="${best.year || yearBe}" placeholder="${yearBe}">
        </div>
      </div>
    </div>
    ${historyHtml}
    ...
    chk.addEventListener('change', function() {
      if (this.checked) {
        ...
        if (time && name) {
          ...
          document.getElementById('bestYear').value = yearBe; // ใช้ปีการศึกษาจาก API
  ...
  document.getElementById('btnSave').onclick = async () => {
    try {
      ...
      const baseUrl = '<?= BASE_URL ?>/referee/';
      const res = await fetch(baseUrl + 'save.php', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload)});
      const out = await res.json();
